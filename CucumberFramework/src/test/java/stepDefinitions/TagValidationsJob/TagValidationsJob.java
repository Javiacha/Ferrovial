...
public class TagValidationsJob {
	...
	public void run_Transfer_trips_jobs() throws Throwable {
		String processExecuted = "SELECT max(jobs_oid) FROM INFO_JOBS_INFODATA where jobs_name = 'Tag Validations Job' and jobs_fec_creation > sysdate - 1/24 order by JOBS_FEC_CREATION asc";
	...
	public void check_Tag_Validations_Job_is_successful() throws Throwable {
		String processExecuted = "SELECT count(1) FROM med_bos_transactions where medtr_oid = 18 and medtr_trnst_fk = 300"; //transacción número proporcionado
		Long cont = 1L;
		Long cont2 = 0L;
		do{
			cont2 = DatabaseConnector.checkResultInDatabase(processExecuted);
